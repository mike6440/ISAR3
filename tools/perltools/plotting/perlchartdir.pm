...
if ($] >= 5.008) {
	use Config;
	my $cdlibext = ($] >= 5.01) ? (($] >= 5.012) ? (($] >= 5.014) ? (($] >= 5.016) ? (($] >= 5.018) ? (($] >= 5.020) ? "520" : "518") : "516") : "514") : "512") : "510") : "58";	if ($Config{"use64bitint"} && (4 < $Config{"ivsize"})) {
		$cdlibext .= "i64";
	}
...
"TTFText"=>undef,
"TextBox"=>undef,
"ThreeDScatterChart"=>undef,
"ThreeDScatterGroup"=>undef,
"TrendLayer"=>undef,
"VectorLayer"=>undef,
"XYChart"=>undef,
...
sub _r
{
	...
	if ($params) {
		...
		my $maker = $$params[0];
		if (defined $maker) {
			return eval(("new $maker('$ret', \$mySelf)" =~ /(.*)/)[0]);
		} else {
			return $ret;
		}		
	} else {
...
sub ClearTypeMono
{
	return _f("ClearTypeMono", \@_, 1, 0);
}
sub ClearTypeColor
{
	return _f("ClearTypeColor", \@_, 1, 0);
}
$CompatAntiAlias = 6;
...
sub phongLighting
{
	return _f("phongLighting", \@_, 4, 0.5, 0.5, 0.75, 8);
}
...
sub getChart 
{
	my ($self, $i) = @{perlchartdir::checkarg(\@_, 2, 0)};
	if ($i == -1) {
...
{
	my $ret = eval {
		my ($self, $contour, $markContour) = @{perlchartdir::checkarg(\@_, 3, 1, "same")};
		if ($markContour == "same") {
			$markContour = $contour;
		}
		perlchartdir::callMethod("ContourLayer.setExactContour", $self->{"this"}, $contour, $markContour);
	};
	if ((!$ret) && ($@)) { perlchartdir::printerr($@); }
}
...
{
	my $ret = eval {
		my ($self, $yCoor, $yAxis) = @{perlchartdir::checkarg(\@_, 3, undef)};
		perlchartdir::callMethod("XYChart.getYValue", $self->{"this"}, $yCoor, perlchartdir::decodePtr($yAxis));
		};
	if ((!$ret) && ($@)) { perlchartdir::printerr($@); }
	return $ret;
}
...
package ThreeDScatterGroup;
@ISA = ("CDAutoMethod");

$defaultArgs = {
	"setDataSymbol4"=>[undef, 4, 11, -1, -1],
	"setDropLine"=>[undef, 2, $perlchartdir::LineColor, 1],
	"setLegendIcon"=>[undef, 3, -1, -1]
};

sub setDataSymbol
{
	if ((scalar(@_) < 3) && ($_[1]) && ($_[1] !~ m/^\d+$/)) { shift()->setDataSymbol2(@_); } 
	else { perlchartdir::_m("ThreeDScatterGroup.setDataSymbol", \@_, 5, 5, -1, -1, 1); }
}
sub setDataSymbol2
{
	if (ref($_[1])) { shift()->setDataSymbol3(@_); } 
	else { perlchartdir::_m("ThreeDScatterGroup.setDataSymbol2", \@_, 1); }
}
sub setDataSymbol3
{
	my $ret = eval {
		my ($self, $d) = @{perlchartdir::checkarg(\@_, 2)};
		perlchartdir::callMethod("ThreeDScatterGroup.setDataSymbol3", $self->{"this"}, $d->{"this"});
		};
	if ((!$ret) && ($@)) { perlchartdir::printerr($@); }
}	

package ThreeDScatterChart;
@ISA = ("ThreeDChart");

$defaultArgs = {
	"addScatterGroup"=>["ThreeDScatterGroup", 8, "", $perlchartdir::CircleSymbol, 5, -1, -1]
};
...
sub new 
{
	my $class = shift;
	my $self = {};
	$self->{"this"} = perlchartdir::_f("ThreeDScatterChart.create", \@_, 5, 
		$perlchartdir::BackgroundColor, $perlchartdir::Transparent, 0);
	return bless($self, $class);
}
...
$defaultArgs = {
	"getSeries2"=>[undef, 6, -1E+308, 1E+308],
	"getDateSeries"=>[undef, 4, 0]
	};
...
sub new 
{
	my $class = shift;
	my $self = {};
	$self->{"this"} = perlchartdir::_f("RanSeries.create", \@_, 1);
	return bless($self, $class);
}
sub DESTROY
{
	my $self = shift;
	perlchartdir::callMethod("RanSeries.destroy", $self->{"this"})
}
sub getSeries
{
	if (defined($_[4])) { return shift()->getSeries2(@_);	} 
	else { return perlchartdir::_m("RanSeries.getSeries", \@_, 3); }
}
...
sub setChartModel {$_[0]->putAttrS(":model", $_[1]); }
sub getChartModel { return $_[0]->getAttrS(":model"); }

sub setFullRange { perlchartdir::_m("WebChartViewer.setFullRange", \@_, 3); }
sub getValueAtViewPort { return perlchartdir::_m("WebChartViewer.getValueAtViewPort", \@_, 3, 0); }
sub getViewPortAtValue { return perlchartdir::_m("WebChartViewer.getViewPortAtValue", \@_, 3, 0); }
sub syncLinearAxisWithViewPort { shift()->syncAxisWithViewPort(@_, 3); }
sub syncLogAxisWithViewPort { shift()->syncAxisWithViewPort(@_, 4); }
sub syncDateAxisWithViewPort { shift()->syncAxisWithViewPort(@_, 5); }
sub syncAxisWithViewPort
{
	my $ret = eval {
		my $self = shift;
		my ($id, $axis, $scaleType) = @{perlchartdir::checkarg(\@_, 3)};
		perlchartdir::callMethod("WebChartViewer.syncAxisWithViewPort", $self->{"this"}, $axis->{"this"}, $id, $scaleType);
		if (!defined $self->{"dependencies"}) { 
			$self->{"dependencies"} = [$axis]; 
		} else {
			push @{$self->{"dependencies"}}, $axis;
		}
	};
	if ((!$ret) && ($@)) { perlchartdir::printerr($@); }
}
