...
sub addCandleStick
{
    ...
    if (scalar(@{$self->{'m_highData'}}) > $self->{'m_extraPoints'}) {
        my $expectedWidth = int(($self->{'m_totalWidth'} - $self->{'m_leftMargin'} -
...
sub addBand
{
    ...
    my $layer = $self->{'m_mainChart'}->addLineLayer2();
    $layer->addDataSet($upperLine, $lineColor, $name);
    $layer->addDataSet($lowerLine, $lineColor);
    return $self->{'m_mainChart'}->addInterLineLayer($layer->getLine(0), $layer->getLine(1),
        $fillColor);
}
...
sub addVolBars2
{
    ...
    my $closeChange = new ArrayMath($self->{'m_closeData'})->delta()->replace(
        $perlchartdir::NoValue, 0)->result();
...
sub addIndicator
{
    ...
    if ($self->{'m_legendFontColor'} != $perlchartdir::Transparent) {
        ...
        $box->setSize($self->{'m_totalWidth'} - $self->{'m_leftMargin'} - $self->{'m_rightMargin'} +
            1, 0);
    }
...
sub addADX
{
    ...
    #initial value
    my $posData = $pos->result();
    my $negData = $neg->result();
    if ((scalar(@$posData) > 1) && ($posData->[1] != $perlchartdir::NoValue) && ($negData->[1] !=
        $perlchartdir::NoValue)) {
...
sub addCCI
{
    my ($self, $height, $period, $color, $deviation, $upColor, $downColor) = @_;
    #typical price
