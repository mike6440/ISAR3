//ulong GetKt15RAD(void) //v13
// go here and convet to RAD in output string

#define		PROGRAMNAME	"isar"
#define		VERSION		"13"
#define		EDITDATE	"160606" 
#define  EEPROM_ID		6
...
//              Add nbb, Nsky, Nocean to eeprom menu
//              Adjust for shutter switches for I1 and i4
...
//v11 15xxxx -- (a) delay the startup pause to 30 sec
//              (b) PointScanDrum from ROSR
//v12 150928 -- (a) PointScanDrum fix.
//v13 160606 -- We have the new KT15 with laser alignment.
//
...
struct idata{
    ...
    long    kt15_target_rad;  	// Digital radiance from the KT15
	...
	******************/
	...
	while (1) {
		...
		while( fabs(fdum-Angle[ipoint]) > 0.5 ) {
			fdum = PointScanDrum(Angle[ipoint]);  //v11
		}
	...
	switch(*cmd)
	{
	   //	menumenu
		...
		case '/':
			...
			puts(" H/h --> Read KT15 target and ambient T");
			...
			puts(" L --> Laser on               l --> Laser off");//v13
		...
		case 'h':
    		...
    		printf("test KT15 RAD=%ld,  ambient temp=%.1f\n", rad,fdum2); //v13
			break;
...
************************************************************************/
{
	...
	rec->kt15_target_rad=MISSING; //v13
    ...
    rec->kt15_target_rad=GetKt15RAD(); //v13
...
*****************************************************************/
{
    ...
    "%ld,%5.1f*00",          			// KT15 radiance and ambient tempartures
	...
	rec->gps[1],       			// cog (Deg)
	rec->gps[2],       			// var (Deg)
    rec->kt15_target_rad,  		// Target radiance from the KT15 (K)  //v13
...
void LaserOn(void)
/*****************************************************
// Send a "LAS ON" command to the KT15
******************************************************/
{
	int i;
	TSerInFlush(IRTTX);
	...
	// to have a \n
	strcpy(cmd,"LAS ON");
    for(i=0;i<strlen(cmd);i++) TSerPutByte(IRTTX, cmd[i]);
	TSerPutByte(IRTTX,'\n');
}
void LaserOff(void)
/*****************************************************
...
//
// History
//v11 -- used rosr s/r. Modified for this code.
//v12 -- small bug fixes
**************************************************************************************/
{
	...
	pos = readEncoder(ee->drumref);
	
	if(pos == MISSING){
		printf("PointScanDrum error. ReadEncoder = MISSING\n");
		return MISSING;
	}
	...
	if( fabs(diff) > ee->SCAN_TOLERANCE )
	{
        ...
        while(fabs(diff) > 5) {
            ...
            if( MilliSecs() - ulTime > 5000 ) {
                printf("PointScanDrum course timeout\n"); //v12
                return pos;
            }
        }
        // NUDGING (W TIMEOUT)
